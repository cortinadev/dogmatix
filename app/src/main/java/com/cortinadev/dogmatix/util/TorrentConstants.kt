...
object TorrentConstants {
     ...
     * It is an inactivity timeout: the clock restarts whenever bytes or peers arrive.
     */
    const val DEFAULT_METADATA_TIMEOUT_S = 30
    const val MIN_METADATA_TIMEOUT_S = 10
    const val MAX_METADATA_TIMEOUT_S = 180
    const val METADATA_POLL_INTERVAL_MS = 500L
    /** Upper bound for a torrent's info dict (libtorrent default is 3 MiB, too small for 10+ TB sets). */
    const val MAX_METADATA_SIZE_BYTES = 256 * 1024 * 1024
